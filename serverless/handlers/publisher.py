...
def handler(event: Dict[str, Any], context):
    """HTTP API handler — publishes the POST body to SNS.
    ...
    region = _region_from_arn(sns_arn) or os.getenv('AWS_REGION', 'us-east-1')
    try:
        client = boto3.client('sns', region_name=region)
        resp = client.publish(TopicArn=sns_arn, Message=body)
